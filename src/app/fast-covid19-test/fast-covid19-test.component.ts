...
export class FastCovid19TestComponent implements OnInit {
    ...
    ngOnInit(): void {        
        ...
        this.TestsForm = this.formBuilder.group({
            ...
            TestData: this.formBuilder.group({
                ...
                BirthDate: this.formBuilder.group({
                    Year: [
                        ...
                        [
                            Validators.required,
                            Validators.max(this.todaysYear),
                            Validators.min(1920),
                        ],
                    ],
                    Month: [
            ...
            SampleData: this.formBuilder.group({
                ...
                SenderName: [
                    { value: this.userFullName, disabled: true },
                    Validators.required,
                ],
                SenderIDnum: [
        ...
        const name = this.MethodCodeArray.filter((option) =>
            option.MethodNumber.includes(method.MethodNumber)
        );
        this.TestsForm.controls.TestData["controls"]["MethodDesc"].setValue(
            name[0].MethodName
        );
        ...
        this.filteredOptions = this.myControl.valueChanges.pipe(
            ...
            map((name) =>
                name ? this._filter(name) : this.CitiesArray.slice()
            )
        );
        this.getCityCodes();
    }
    ...
    private _filter(value: string): City[] {
        ...
        return this.CitiesArray.filter((option) =>
            option.cityName.includes(filterValue)
        );
    }

    changeCity(event) {
        this.TestsForm.controls.SampleData["controls"]["CityCode"].setValue(
            event.cityCode
        );
        this.TestsForm.controls.SampleData["controls"]["CityDesc"].setValue(
            event.cityName
        );
    }

    completeResultText(event) {
        this.TestsForm.controls.TestData["controls"]["Result"].setValue(
            event.TestName
        );
    }
    ...
    completeMethodText(event) {
        ...
        const name = this.MethodCodeArray.filter((option) =>
            option.MethodNumber.includes(filterValue)
        );
        this.TestsForm.controls.TestData["controls"]["MethodDesc"].setValue(
            name[0].MethodName
        );
    ...
    checkBirthDateValidity() {
        if (
            this.TestsForm.controls.TestData["controls"]["BirthDate"].controls
                .Year.value === this.todaysYear
        ) {
            this.TestsForm.controls.TestData["controls"][
                "BirthDate"
            ].controls.Month.setValidators(
                Validators.max(this.todaysMonth + 1)
            );
            this.TestsForm.controls.TestData["controls"][
                "BirthDate"
            ].controls.Day.setValidators(Validators.max(this.todaysDay));
        } else {
            this.TestsForm.controls.TestData["controls"][
                "BirthDate"
            ].controls.Month.setValidators(Validators.max(12));
            this.TestsForm.controls.TestData["controls"][
                "BirthDate"
            ].controls.Day.setValidators(Validators.max(31));
        }
    }
        ...
        this.http
            ...
            .subscribe((Response) => {
                ...
                this.TestsForm.controls.SampleData["controls"][
                    "SenderName"
                ].setValue(this.userFullName);
            });
    }

    is_israeli_id_number() {
        let id = String(
            this.TestsForm.controls.TestData["controls"]["IDNum"].value
        ).trim();
    ...
    sendReport() {
        if (!this.TestsForm.invalid) {
            this.TestsForm.controls.TestData["controls"][
                ...
                this.TestsForm.controls.TestData["controls"]["ResultTestCorona"]
                    .value.TestNumber
            );
...
LastName
            */
                        this.PatienId = this.TestsForm.value.TestData.IDNum;
                        this.FirstName =
                            this.TestsForm.value.TestData.FirstName;
                        this.LastName = this.TestsForm.value.TestData.LastName;
                        this.CheckResuls = this.TestsForm.value.TestData.Result;
                        var date = new Date();

                        // Add a day
                        date.setDate(date.getDate() + 1);
                        this.dateEnd = this.datePipe.transform(
                            date,
                            "dd.MM.yyyy"
                        );
                        ////debugger
    ...
    print() {
        ...
        setTimeout(function () {
            ...
            mywindow.document.write(
                "<html><head><title>בדיקת קורונה מהירה</title>"
            );
            mywindow.document.write(
                '</head><body dir="rtl" style="text-align:right;width:300px;margin:auto;display:block;">'
            );
            mywindow.document.write(
                '<h1 style="text-align:center">בדיקת קורונה מהירה</h1>'
            );
            mywindow.document.write(
    ...
    getCategories() {
        this.http
            ...
            .subscribe((Response) => {
                ...
                this.subCategory =
                    this.all_categories_filter[lastIndex].SubCategory;
            });
    }
